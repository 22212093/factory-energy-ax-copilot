...
// REPORT_TXT_DOWNLOAD_HELPER_START
function getReportDownloadLabels(language) {
  const labels = {
    ko: {
    ...
    de: {
      ...
      filePrefix: 'AI_Report',
      title: 'KI-Analysebericht',
      equipment: 'Anlage',
      eventTime: 'Ereigniszeit',
      confidence: 'Konfidenz',
      savings: 'Erwartete Einsparung',
      summary: 'Zusammenfassung',
      causes: 'Geschätzte Ursachen',
    ...
    id: {
      ...
      savings: 'Estimasi Penghematan',
      summary: 'Ringkasan',
      causes: 'Perkiraan Penyebab',
      actions: 'Tindakan Rekomendasi',
      source: 'Sumber Analisis',
    },
  };
...
function formatReportAsText(report, event, language, source) {
  const l = getReportDownloadLabels(language);
  const sourceLabel = source === 'gemini' ? 'Gemini Live' : 'Local Rule';
  const equipment = event?.equipment || report?.title || 'AI Report';
  ...
  lines.push('========================================');
  lines.push('');
  lines.push(l.source + ': ' + sourceLabel);
  lines.push(l.equipment + ': ' + equipment);
  lines.push(l.eventTime + ': ' + eventTime);
  lines.push(l.confidence + ': ' + (report?.confidence ?? '-') + '%');
  lines.push(l.savings + ': ' + (report?.savings || '-'));
  lines.push('');
  ...
  (report?.causes || []).forEach((cause, index) => {
    lines.push(String(index + 1) + '. ' + (cause.label || '-'));
    if (cause.detail) lines.push('   - ' + cause.detail);
  });
  lines.push('');
  lines.push('[' + l.actions + ']');
  (report?.actions || []).forEach((action, index) => {
    lines.push(String(index + 1) + '. ' + (action.label || '-'));
  ...
  // RAG Evidence Documents
  if (report?.ragDocs && report.ragDocs.length > 0) {
    const ragLabel = language === 'ko' ? 'RAG 근거 문서' : 'RAG Evidence Documents';
    lines.push('[' + ragLabel + ']');
    report.ragDocs.forEach((doc, index) => {
      lines.push(String(index + 1) + '. ' + (doc.title || '-'));
      if (doc.desc) lines.push('   - ' + doc.desc);
      if (doc.url) lines.push('   - URL: ' + doc.url);
    });
    lines.push('');
  }

  lines.push('Generated by Factory Energy AX Copilot');

  return lines.join('\n');
}

function downloadReportText(report, event, language, source) {
  const l = getReportDownloadLabels(language);
  const equipment = safeFileNamePart(event?.equipment || report?.title || 'AI_Report');
  const stamp = new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-');
  const filename = l.filePrefix + '_' + equipment + '_' + stamp + '.txt';

  const content = formatReportAsText(report, event, language, source);
...
function ReportDownloadButton({ report, event, language, source }) {
  ...
  return (
    <button
      type="button"
      onClick={() => downloadReportText(report, event, language, source)}
      style={{
        display: 'flex',
        alignItems: 'center',
        gap: '5px',
...
// REPORT_TXT_DOWNLOAD_HELPER_END
// ─── Main export ───────────────────────────────────────────
